...
describe('Integration test for the BMICalculator', () => {
  test.each([
    ...
    const actualBMIValue = Number(CalculateBMI(mockUserWeight, mockUserHeight))
    const actualBMIStatus = PrintBMIStatus(actualBMIValue)
    const actualStatusInfo = PrintBMIInfo(actualBMIStatus)
